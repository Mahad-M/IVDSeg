"""Train B4's ImageNet-pretrained Swin V2 Small U-Net on development data only."""
...
from ivdseg.unet_training import (
    B4_DEVELOPMENT_DICE_METRIC,
    B4DevelopmentConfig,
    build_b4_development_training,
    write_b4_development_configuration,
)
...
def _write_run_card(path: Path, config: B4DevelopmentConfig, command: str) -> None:
    if path.exists():
        raise FileExistsError(f"experiment card already exists: {path}; use a new experiment ID or seed")
    content = f"""# {config.experiment_id} - Pretrained 12-channel Swin V2 Small U-Net - Seed {config.seed}
...
- **Hypothesis:** Fine-tuning an ImageNet-pretrained Swin V2 Small encoder with a U-Net decoder improves B3's development segmentation performance through deeper hierarchical attention.
- **Comparison:** B4 changes B3's pretrained Swin V2 Tiny encoder depth `(2,2,6,2)` to pretrained Swin V2 Small depth `(2,2,18,2)`. It retains the development split, normalization, 12-channel tensor, `384 x 384` resize, semantic labels, augmentation, seed, objective, and subject-volume development selection. Its micro-batch is reduced to one with eight accumulation steps to retain effective batch size eight within the L4 memory budget; encoder LR is reduced to `5e-5` for the larger pretrained encoder.
...
- **Model and initialization:** `SwinV2SmallUNet(in_channels=12, out_channels=1)` with torchvision `Swin_V2_S_Weights.IMAGENET1K_V1` ImageNet weights. Its pretrained RGB patch projection is repeated across the four 3-channel modality groups and divided by four; the three skip-fusion and two full-resolution decoder stages are new.
...
- **Pretrained encoder cache:** `ivdseg-runs:/artifacts/model-weights/hub/checkpoints/swin_v2_s-637d8ceb.pth` (downloaded from PyTorch only if absent).
...
- **Decision / next action:** Select B4's sigmoid threshold only from the best development checkpoint using subjects `02,12`; final retraining and fixed-holdout evaluation remain prohibited until that decision is recorded.
"""
...
def main() -> None:
    ...
    parser.add_argument("--experiment-id", default="B4-swinv2-small-pretrained")
    ...
    try:
        verify_cuda_runtime()
        trainer, module, data_module, checkpoint = build_b4_development_training(config)
        config_path = write_b4_development_configuration(config, module.model)
        ...
        _finish_run_card(
            ...
            status="Complete",
            conclusion=(
                f"Development training completed. Best {B4_DEVELOPMENT_DICE_METRIC}={score_text}; configuration is "
